...
uint8_t mem_access(vaddr_t addr, char access, uint8_t data) {
    ...
    stats.accesses++;
    vpn_t vpn_pointer = vaddr_vpn(addr);
    pte_t *pte_pointer = vpn_pointer + (pte_t *)((PTBR * PAGE_SIZE) + mem);
    
    if(!pte_pointer->valid) {
        page_fault(addr);
    }
    
    frame_table[pte_pointer->pfn].mapped = 1;
    paddr_t p_add = (paddr_t)((pte_pointer->pfn << OFFSET_LEN) | vaddr_offset(addr));
    pte_pointer->referenced = 1;
