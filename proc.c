...
void proc_init(pcb_t *proc) {
    ...
    pfn_t new_frame = free_frame(); // gets the frame number of a new frame
    memset(mem + (new_frame * PAGE_SIZE), 0, PAGE_SIZE); // clearing section for page table

    fte_t* m = (frame_table + new_frame);
    
    proc->saved_ptbr = new_frame; // update ptbr

    m->process = proc; // appropiate flags
    m->protected = 1;
}
...
void proc_cleanup(pcb_t *proc) {
   ...
   pte_t *process_page_table = (pte_t *) (mem + (proc->saved_ptbr * PAGE_SIZE));

    // Iterate over each page table entry
    for (size_t page_index = 0; page_index < NUM_PAGES; page_index++) {
        pte_t *current_pte = process_page_table + page_index;
        
        // If the page is valid, update frame table and reset the page entry
        if (current_pte->valid) {
            frame_table[current_pte->pfn].mapped = 0;
            current_pte->valid = 0;
            current_pte->dirty = 0;
        }
        
        // If the page has been swapped, free the swap space
        if (swap_exists(current_pte)) {
            swap_free(current_pte);
        }
    }

    // Unmap and unprotect the frame holding the page table
